    ...
    if os.name == "posix":
        ...
        if distro.id().lower() in ["ubuntu", "debian", "deepin", "darwin"]:
            ...
            if is_rss_variant_enabled():
                ...
                extra_link_args = [
                    os.path.join(pwd, "dependencies/lib/libcarla_client_rss.a")
                ]
            else:
                extra_link_args = [
            ...
            extra_compile_args = [
                "-isystem",
                os.path.join(pwd, "dependencies/include/system"),
                "-fPIC",
                "-std=c++14",
                "-Werror",
                "-Wall",
            ...
            if is_rss_variant_enabled():
                ...
                extra_compile_args += [
                    "-DLIBCARLA_PYTHON_MAJOR_" + str(sys.version_info.major)
                ]
                extra_link_args += [
                    os.path.join(
                        pwd,
                        "dependencies/lib/libad_rss_map_integration_python"
                ...
                extra_link_args += [
                    os.path.join(
                        pwd,
                        "dependencies/lib/libad_map_access_python"
                        + str(sys.version_info.major)
                        + str(sys.version_info.minor)
                        + ".a",
                    )
                ...
                extra_link_args += [
                    os.path.join(
                        pwd,
                        "dependencies/lib/libad_rss_python"
                        + str(sys.version_info.major)
                        + str(sys.version_info.minor)
                        + ".a",
                    )
                ...
                extra_link_args += [
                    os.path.join(
                        pwd,
                        "dependencies/lib/libad_physics_python"
                        + str(sys.version_info.major)
                        + str(sys.version_info.minor)
                        + ".a",
                    )
            ...
            raise NotImplementedError(
                linux_distro
                + " not in supported posix platforms: "
                + str(supported_dists)
            )
    elif os.name == "nt":
        pwd = os.path.dirname(os.path.realpath(__file__))
        pylib = "libboost_python%d%d" % (sys.version_info.major, sys.version_info.minor)
        ...
        libs = [
            x
            for x in os.listdir("dependencies/lib")
            if any(d in x for d in required_libs)
        ]
